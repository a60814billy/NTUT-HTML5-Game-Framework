var Framework = (function (Framework) {
	...
	Framework.KeyBoardManager = (function(){
		...
		var keydownEvent = function(e) {
			...
			userKeydownEvent.call(this, keydownList[keyCode], keydownList, e);
		};

		var keyupEvent = function(e) {
			e.preventDefault();
			var keyCode = keyCodeToChar[e.which || e.keyCode], temp = {};
			keypressHistory.push(keydownList[keyCode]);
			keydownList[keyCode] = null;
			delete keydownList[keyCode];
			for(temp in keydownList) {
				if(Framework.Util.isUndefined(keydownList[temp][keyCode])) {
					return;
				}
				keydownList[temp][keyCode] = false;
			}
		...
		KeyBoardManagerClass.prototype = {
			...
			keypressHistory: keypressHistory,	/* 為了要可以做出類似小朋友齊打交的功能(一定時間內, 連續按多個鍵後, 會有招式) */
		}
